...
//            $this->show_message( $this->messages['updated_paid_to_free'] , 20 );    
    ...
    private function get_formated_message ( $message, $message_type = 'updated', $inner_message_id = '') {
        ...
        $notice .= '<a class="close tooltip_left" rel="tooltip" title="'. esc_js(__("Hide" ,'booking')). '" data-dismiss="alert" href="javascript:void(0)" onclick="javascript:void(0)">&times;</a>';

        // $my_close_open_alert_id = 'wpbc_system_message_id_';
        // $notice .= '<a class="close tooltip_left" rel="tooltip" title="'. esc_js(__("Don't show the message anymore" ,'booking')). '" data-dismiss="alert" href="javascript:void(0)" onclick="javascript:verify_window_opening('. get_bk_current_user_id() .', \''. $my_close_open_alert_id .'\');">&times;</a>';
